...
class ActorFixSAC(nn.Module):
    ...
    def get_logprob(self, state, action):
        ...
        log_prob = action_std_log + self.log_sqrt_2pi + noise.pow(2).__mul__(0.5)  # noise.pow(2) * 0.5
    ...
    def get_action_logprob(self, state):
        ...
        log_prob = Normal(action_avg, action_std).log_prob(a_noise)
        # same as below:
        a_delta = (action_avg - a_noise).pow(2) /(2*action_std.pow(2))
        log_prob = -a_delta - action_std.log() - np.log(np.sqrt(2 * np.pi))
        """
...
class ActorPPO(nn.Module):
    def __init__(self, mid_dim, num_layer, state_dim, action_dim):
        super().__init__()
        self.net = build_mlp(mid_dim, num_layer, input_dim=state_dim, output_dim=action_dim)
        state = torch.ones((2, state_dim), dtype=torch.float32)
    ...
    def get_logprob(self, state, action):
        ...
        delta = ((action_avg - action) / action_std).pow(2) * 0.5
        log_prob = -(self.action_std_log + self.sqrt_2pi_log + delta)  # new_logprob
        return log_prob
