...
class AgentBase:  # [ElegantRL.2022.04.04]
    def __init__(self, net_dim: int, state_dim: int, action_dim: int, gpu_id=0, args=None):
        ...
        self.h_term_rm_rate = getattr(args, 'h_term_rm_rate', 2 ** -2)  # remove the data in H-term ReplayBuffer
        self.h_term_bs_rate = getattr(args, 'h_term_bs_rate', 2 ** -2)  # sample the data in H-term ReplayBuffer
        self.h_term_buffer = list()
        self.ten_state = None
        self.ten_action = None
        self.ten_r_sum = None
        self.ten_r_norm = None
        self.criterion1 = torch.nn.SmoothL1Loss(reduction="none")
    ...
    def get_obj_critic_raw(self, buffer, batch_size) -> tuple:
        """
    ...
    def get_obj_critic_per(self, buffer, batch_size) -> tuple:
        """
    ...
    def get_buf_h_term(self, buf_state, buf_action, buf_r_sum):
        ...
        h_term_throw = max(0, int(len(self.h_term_buffer) * self.h_term_rm_rate) - 1)
