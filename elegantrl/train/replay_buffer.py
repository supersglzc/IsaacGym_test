...
class ReplayBuffer:  # for off-policy
    ...
    def update_buffer(self, traj_list):
        ...
        p = self.next_p + rewards.shape[0]  # pointer
    ...
    def sample_batch_r_m_a_s(self):
        if self.prev_p <= self.next_p:
            reward = self.buf_reward[self.prev_p:self.next_p]
            mask = self.buf_mask[self.prev_p:self.next_p]
            action = self.buf_action[self.prev_p:self.next_p]
            state = self.buf_state[self.prev_p:self.next_p]

        else:
            reward = torch.vstack((self.buf_reward[self.prev_p:],
                                   self.buf_reward[:self.next_p]))
            mask = torch.vstack((self.buf_mask[self.prev_p:],
                                 self.buf_mask[:self.next_p]))
            action = torch.vstack((self.buf_action[self.prev_p:],
                                   self.buf_action[:self.next_p]))
            state = torch.vstack((self.buf_state[self.prev_p:],
                                  self.buf_state[:self.next_p],))
        self.prev_p = self.next_p
        return reward, mask, action, state
